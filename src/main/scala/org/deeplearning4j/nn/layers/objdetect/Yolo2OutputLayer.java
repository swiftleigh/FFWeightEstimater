...
public class Yolo2OutputLayer extends AbstractLayer<org.deeplearning4j.nn.conf.layers.objdetect.Yolo2OutputLayer> implements Serializable, IOutputLayer {
    ...
    private INDArray computeBackpropGradientAndScore(){
        ...
        double lambdaCoord = layerConf().lambdaCoord;
        double lambdaNoObj = layerConf().lambdaNoObj;

        int mb = input.size(0);
        int h = input.size(2);
        int w = input.size(3);
        int b = layerConf().boundingBoxes.size(0);
        ...
        INDArray boxPriorsNormalized = layerConf().boundingBoxes.divRowVector(wh); //Shape: [b, 2] - Normalize box priors by width and height
...
//        Broadcast.div(labelCenterXYImg, wh, labelCenterXYImg, 1);
        ...
        double positionLoss = layerConf().lossPositionScale.computeScore(labelXYCenter2d, predictedXYCenter2d, identity, mask1_ij_obj_2d, false );
        double sizeScaleLoss = layerConf().lossPositionScale.computeScore(labelWHSqrt2d, predictedWHSqrt2d, identity, mask1_ij_obj_2d, false);
        ...
        double classPredictionLoss = layerConf().lossClassPredictions.computeScore(classLabels2d, classPredictionsPreSoftmax2d, new ActivationSoftmax(), mask2d, false);
        ...
        INDArray gradPredictionLoss2d = layerConf().lossClassPredictions.computeGradient(classLabels2d, classPredictionsPreSoftmax2d, new ActivationSoftmax(), mask2d);
        ...
        INDArray gradXYCenter2d = layerConf().lossPositionScale.computeGradient(labelXYCenter2d, predictedXYCenter2d, identity, mask1_ij_obj_2d);
        ...
        INDArray gradWHSqrt2d = layerConf().lossPositionScale.computeGradient(labelWHSqrt2d, predictedWHSqrt2d, identity, mask1_ij_obj_2d);   //Shape: [mb*b*h*w, 2]
    ...
    public INDArray activate(boolean training) {
        ...
        INDArray priorBoxes = layerConf().boundingBoxes;   //Shape: [B, 2]
    ...
    public static class IOURet {
        ...
        public INDArray iou;
        public INDArray dIOU_dxy;
        public INDArray dIOU_dwh;
